...
export default function User() {
  ...
  const updateOrganizer = async (body: any) => {
    ...
    setOrganizers(organizers.map((organizer) => organizer.organizerId === body.organizerId ? body : organizer));
  ...
  const deleteOrganizer = async (id: number) => {
    ...
    if (response.status === 200) {
      ...
      setOrganizers(organizers.filter((organizer) => organizer.organizerId !== id));
    } else {
...
const UpdateForm: React.FC<{ updateOrganizer: (updateBody: Organizer) => void, updateBody: any }> = ({ updateOrganizer, updateBody }) => {
